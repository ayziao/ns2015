	...
	form.parse(request, function (err, fields, files) {
		...
		if (typeof fields.body !== "undefined" && fields.body.trim() !== "") {
			...
			ns.post(body, taglist, files, user, function (err) {
				if (err) {
					_returnResponse(response, 500, '500' + httpStatus[500], {type: 'txt'}, logs);
					_errorLog({_httpPost: '投稿失敗', err: err});
				} else {
//					_returnResponse(response,200,'posted',{type:'txt'},logs);
					_redirect(response, 303, _redirectUrl, logs);
				}
			});
		} else { //本文無しは転送
			_redirect(response, 303, _redirectUrl, logs);
...
//転送
function _redirect(response, statusCode, url, logs) {
 ...
 * 
 * @param {type} request
 * @returns {_requestFromUser.arrayOfStrings|String}
 */ //リクエストヘッダからユーザ取得
