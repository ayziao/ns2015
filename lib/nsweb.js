	...
	if (requestUrl === '/') { //トップへのアクセス
		...
		ns.timeline(user, '/', 'html', function (err, content) {
			...
			fs.readFile(formHtmlPath, 'utf8', function (err, formHtml) {
				...
				var replace = {urlbase: URL_BASE, user: user};
				if (parseurl.query.form) {
					replace.textval = parseurl.query.form;
				}
				formHtml = templateReplace(formHtml, replace ,true);
				var reshtml = content.replace("__form__",formHtml);
				returnResponse(response, 200, reshtml, {type: 'html'}, logs);
			});
		});

	} else if (requestUrl.search(/^\/\d{8}$/) === 0) { //数字のみ8個
		...
		ns.daysummary(user, requestUrl.slice(1), 'html', function (err, content) {
			...
			var reshtml = content.replace("__form__",'');
			returnResponse(response, 200, reshtml, {type: 'html'}, logs);
		});
		...
		ns.content(user, requestUrl, extname, function (err, content, contentStatus) {
			if (err){ //エラー
				...
				returnResponse(response, 500, '500' + httpStatus[500], contentStatus, logs);
				errorLog({httpGet: 'content 取得エラー', err: err , contentStatus:contentStatus});
				return;
			}
			if (!content) { //コンテントがない場合404
				contentStatus.requestType = contentStatus.type;
				contentStatus.type = 'txt';
				returnResponse(response, 404, '404' + httpStatus[404], contentStatus, logs);
				return;
			}
...
function httpPost(request, response, logs) {
	...
	form.parse(request, function (err, fields, files) {
		...
		if (typeof fields.body !== "undefined" && fields.body.trim() !== "") {
			...
			var taglist = fields.tags.trim().replace(/\s/, " ").replace(/\s{2,}/, " ")
					.split(" ").filter(Boolean);
			ns.post(body, taglist, files, user, function (err) {
				if (err) {
					returnResponse(response, 500, '500' + httpStatus[500], {type: 'txt'}, logs);
