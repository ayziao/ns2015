				...
				if (type === 'html') {
					...
					fs.readFile(tophtmlPath, 'utf8', function (err, tophtml) {
						if (err) {
							...
							//FIXME ユーザ指定文字列埋め込み
							if (userinsert[user]) {
								replace.userinsert = userinsert[user];
							} else {
								replace.userinsert = "<!-- userinsert -->";
							}
			...
			db.serialize(function () {
				...
				db.all(sql, function (err, rows) {
					if (err) {
						...
						callback(err, null, contentStatus);
						return;
					}
					...
					if(type === 'html'){
						...
						fs.readFile(tophtmlPath, 'utf8', function (err, tophtml) {
							if (err) {
								...
								var replace = {user: user, title: path.slice(1) ,contents:timeline};
								//FIXME ユーザ指定文字列埋め込み
								if (userinsert[user]) {
									replace.userinsert = userinsert[user];
								} else {
