...
function timelinekumitate(rows) {
	...
	rows.forEach(function (row) {
		...
		taglist = row.tags.trim().split(' ').filter(Boolean);
		taglist.forEach(function (tag) {
			if (tag.substr(0, 1) === '#') {
				timelineHtml += ' <a href="./?tag=' + tag.substr(1) + '">' + tag.substr(1) + '</a>';
			}
		});
		timelineHtml += "</div>\n";
	});
...
//検索
function searchbody(user, str, type, callback) {
	var replaceList, rtnhtml, timelineHtml, tophtmlPath;
	db.serialize(function () {
		var sql = "SELECT * FROM basedata WHERE user = '" + user
				+ "' AND tags NOT LIKE '% gyazo_posted %' "
				+ " AND body LIKE '%" + str + "%' ORDER BY identifier ASC LIMIT 1000";
		db.all(sql, function (error, rows) {
			if (!error) {
				if (type === 'html') {
					timelineHtml = timelinekumitate(rows);
...
//タグ検索
function searchtag(user, str, type, callback) {
	var replaceList, rtnhtml, timelineHtml, tophtmlPath;
	db.serialize(function () {
		var sql = "SELECT * FROM basedata WHERE user = '" + user
				+ "' AND tags NOT LIKE '% gyazo_posted %' "
				+ " AND tags LIKE '%" + str + "%' ORDER BY identifier ASC LIMIT 1000";
		db.all(sql, function (error, rows) {
			if (!error) {
				if (type === 'html') {
					timelineHtml = timelinekumitate(rows);
										...
										db.run(basedataInsertSql,
												...
												function (dberror) {
													if (dberror) {
														...
														//マルチポスト
														mp.post(body, tags, files, gyazores, user);
													}
												}
										);
									}
