 ...
 * @param {type} value 削除対象の値
 * @returns {Array} 値を削除した配列
 */
 ...
 * 日付をフォーマットする
 * @param  {Date}   date     日付
 * @param  {String} [opt_format='YYYY-MM-DD hh:mm:ss.SSS wd'] フォーマット
 * @return {String}          フォーマット済み日付
 *
 */
function formatDate(date, opt_format) {
	var format, i, length, milliSeconds;
 ...
 * @param {type} tpl
 * @param {type} replace
 * @param {type} replaceStrDelete
 * @returns {templateReplace.ret}
 */
function templateReplace(tpl, replace, replaceStrDelete) {
	var found, key, target;
	var ret = tpl;

	if (replaceStrDelete) {
		//文字列から__[半角英数字(_含む)列]__探して重複取って控える
		found = ret.match(/__\w+__/g).filter(function (x, i, self) {
	...
	for (key in replace) {
		if (replace.hasOwnProperty(key) && key.search(/^\w+$/) === 0) {
			...
			if (replaceStrDelete) {
				//置換した __*＿ を控えから削除
				target = '__' + key + '__';
				found = arrayDeleteValue(found, target);
			}
		}
	}
 ...
 */
module.exports = {
	arrayDeleteValue: arrayDeleteValue,
